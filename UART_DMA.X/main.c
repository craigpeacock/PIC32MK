 ...
 * Peripheral DMA Example. Copies data from UART to RAM RecvBuffer. Once 8 bytes
 * have been received, prints buffer to screen.
 * 
...
void init_DMA(void)
{
    ...
    DCH0SSA = KVA_TO_PA((void *)&U2RXREG);      // Source
    DCH0DSA = KVA_TO_PA((void *)RecvBuffer);    // Destination
    DCH0SSIZ = 1;                   // Source Size (bytes)
    DCH0DSIZ = sizeof(RecvBuffer);  // Destination Size (bytes)
    DCH0CSIZ = 1;                   // Bytes transferred per trigger event. 
                                    // UART contains one byte (We effectively 
                                    // disable UART's FIFO for this example)
    DCH0CONbits.CHAEN = 1;          // Auto-enabled
    
    DCH0ECONbits.CHSIRQ = _UART2_RX_VECTOR; // Channel Transfer start IRQ 
...
void __ISR (_DMA0_VECTOR, IPL2AUTO) DMA0_Handler(void)
{
    if (DCH0INTbits.CHDDIF) {
        printf("\r\nDMA0 Channel Destination Done Interrupt\r\nBuffer: [");
        // Print out receive buffer:
        for (int i = 0; i < MAX_BUFFER; i++)
            putchar(RecvBuffer[i]);
        printf("]\r\n\r\n");
        DCH0INTbits.CHDDIF = 0;         // Clear Flag
    }
    IFS2bits.DMA0IF = 0;       // Clear Flag
}
...
void init_UART2(void)
{
    ...
    ANSELGbits.ANSG7 = 0;       // Make RPG7 Digital
    U2RXRbits.U2RXR = 0b1010;   // Receive on RPG7 (Input)
    ...
    U2STAbits.URXISEL = 0b00;   // Interrupt with 1 character
...
void __ISR (_UART2_RX_VECTOR, IPL2AUTO) U2RX_Handler(void)
{
    printf("UART2 RX Interrupt: \r\n");
    
    //Echo character to console:
    //The DMA controller may have fetched the byte, hence URXDA is cleared
